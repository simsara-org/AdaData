...
# Simsara Token Tool
# ------------------
# A helper script for creating Cardano native assets and metadata.
#
...
# Please keep this header and attribution.
#
...
# Inputs  : none
# Outputs : ./keys/policy.skey, ./keys/policy.vkey, ./policy/policy.script
# Security: Keep all output files offline; never commit keys to a repo!
# ===========================================================================
...
prompt_asset_name() {
    ...
    echo "$ASSET_NAME"     > asset_name.txt
    echo "$ASSET_NAME_HEX" > asset_name_hex.txt
    echo "$ASSET_ID"       > asset_id.txt
}
...
prompt_metadata() {
    ...
    DEFAULT_LOGO="/app/default_logo.png"   # bundle a small placeholder image
        ...
        case "$(echo "${LOGO_CHOICE:-Y}" | tr '[:lower:]' '[:upper:]')" in
            Y)
                ...
                if [ -n "$LOGO_PATH" ] && [ -f "$LOGO_PATH" ]; then
                    ...
                    base64 "$LOGO_PATH" > "$LOGO_BASE64_OUT"
                ...
                if [ -z "$USER_LOGO" ]; then
                    if [ -f "$DEFAULT_LOGO" ]; then
                        ...
                        base64 "$DEFAULT_LOGO" > "$LOGO_BASE64_OUT"
                    else
                        echo "No default logo available; skipping."
                    fi
                    break
                elif [ -f "$USER_LOGO" ]; then
                    base64 "$USER_LOGO" > "$LOGO_BASE64_OUT"
                    echo "Logo base64 saved to $LOGO_BASE64_OUT"
                    break
                else
...
sign_token_metadata_json() {
    ...
    case "${create_signed^^}" in
        Y)
            ...
            if [ -f logo_base64.txt ]; then
                # token-metadata-creator expects a png file, not base64: for registry you submit the png!
                # So: ask the user for original PNG path again
                while true; do
                    echo -n "Logo detected. Please provide path to your logo PNG file for registry (must match original!): "
                    read LOGO_PNG
                    if [ -f "$LOGO_PNG" ]; then
